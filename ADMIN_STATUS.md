...
### 3. 예약관리 — DONE (Mock)
- `/admin/reservations`: 테이블/모바일 카드, **상태 필터**(전체/대기/확정/완료/노쇼/취소·거절) + **필터별 건수 배지**, **프로그램 필터**(드롭다운), **검색**(고객명·연락처·예약번호·프로그램, 지우기 버튼), **결과 합계 + 필터 초기화**, 행별 액션.
...
- TODO: 실제 DB 트랜잭션, 알림 발송, 기간(날짜 범위) 필터.

### 4. 고객관리 — PARTIAL
- `/admin/customers`: 목록 + **검색**(이름·연락처) + **정렬**(예약많은순/최근방문/이름) + **세그먼트 필터**(전체/단골5회+/일반) + **결과 합계** **DONE**, 단골 배지.
...
### 6. 프로그램관리 — PARTIAL
- `/admin/programs`: 목록 + **검색**(프로그램명) + **정렬**(기본/가격↑↓/이름) + **상태 필터**(전체/노출중/숨김) + **결과 합계** + 등록/수정 폼(zod) + **노출 토글** **DONE**.
...
| 지표 | 점수 | 근거 |
|---|---|---|
| **Admin UI %** | **94%** | 7개 메뉴 + 공지 모두 화면 완성, 반응형·토스트·로딩 처리. 예약(건수배지·프로그램필터·합계)·고객(세그먼트)·프로그램(검색·상태필터) 탐색 UX 강화. 고객 상세/이미지 업로더 UI만 미완 |
| **Admin Function %** | **70%** | 예약·일정·프로그램 변이 액션 동작(Mock). 검색/필터/정렬은 클라이언트 처리로 완비. 미완: 실인증, 설정 저장 배선, 고객 상세, 이미지 업로드 |
...
## 부록 — 2026-06-07 추가된 관리자 탐색 UX (Mock 범위)
> 우선순위 4 "관리자 UX 개선"의 산출물. 모두 클라이언트 측, Mock 데이터로 동작.
- **예약 관리**: 상태 탭별 건수 배지, 프로그램 드롭다운 필터, 검색어 지우기(X), "총 N건" 합계 + 필터 초기화.
- **고객 관리**: 세그먼트 필터(전체/단골5회+/일반), "총 N명" 합계.
- **프로그램 관리**: 프로그램명 검색, 상태 필터(전체/노출중/숨김), "총 N개" 합계, 빈 결과 안내.
